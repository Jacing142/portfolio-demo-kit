...
def _identifier_like(value):
    ...
    parts = value.split()
    if len(parts) == 1:
        # One token: an ID or email has a digit or "@"; a name is capitalised. A lowercase word
        # such as "processing" is a category value, not an identifier.
        return any(c.isdigit() for c in value) or "@" in value or value[:1].isupper()

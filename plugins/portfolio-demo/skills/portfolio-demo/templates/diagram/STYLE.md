...
1. **Show only confirmed steps.** A demo diagram shows only the steps the code performs and the steps the owner confirmed in phase 2. If a step happens outside the code (for example "a person reviews every draft"), it appears only if the owner said so, in the owner's words.

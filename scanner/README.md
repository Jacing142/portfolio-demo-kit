...
public in this kit's `templates/` folder, and model names such as `claude-sonnet-5`.

## Commands

```bash
python3 -m scanner plant --src input/ --dst work/copy --out ../private/canaries.json
python3 -m scanner scan --source work/copy --output output/ \
    --canaries ../private/canaries.json --allow work/decisions.json \
    [--paraphrase] [--report work/scan/report.json] [--markdown work/scan/report.md]
python3 -m scanner patterns some-file-or-folder     # secret patterns only
```

Exit codes: `0` clean, `1` leaks found, `2` self-test failed or bad input.
`--report` holds the matched text in full: keep it private. `--markdown` masks every match.

## Tests

```bash
python3 -m unittest discover -s scanner/tests -t .
```

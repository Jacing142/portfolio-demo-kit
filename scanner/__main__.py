"""Command line for the leak scanner.

  python3 -m scanner plant    --src input/ --dst work/copy --out work/canaries.json
  python3 -m scanner scan     --source work/copy --output output/ [--canaries ...] [--allow ...]
  python3 -m scanner patterns FILE_OR_FOLDER    (secret patterns only, e.g. on the issue text)
...
def main(argv=None):
    ap = argparse.ArgumentParser(prog="python3 -m scanner")
    ...
    s.add_argument("--public", action="append", default=None,
                   help="folder of already-public text to ignore (default: templates/)")
